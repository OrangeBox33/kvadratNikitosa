import { addMessage, setChat, setGrid, setPalette, setPixels } from '../redux/slice';
...
socket.addEventListener('message', (message) => {
	const { type, pixels, grid, chat, chatMessage, palette } = JSON.parse(message.data);
	...
	if (type === EMessageTypes.GET_PALETTE) {
		store.dispatch(setPalette(palette));
	}
});
